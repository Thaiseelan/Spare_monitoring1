...
const SensorDetails: React.FC = () => {
    ...
    const fetchTimeBasedData = async () => {
        ...
        try {
            const [timeData, aggData] = await Promise.all([
                apiService.getTimeBasedSensorData(parseInt(componentId), 'daily'), // Assuming 'daily' is the default for now
                apiService.getAggregatedSensorData(parseInt(componentId), 'daily')
            ]);
            ...
            if (timeData.data && timeData.data.length > 0) {
                const chartData = timeData.data.slice(-10).map((item: any) => ({
                    time: new Date(item.timestamp).toLocaleTimeString(),
                    value: item.temperature
                }));
                setTemperatureData(chartData);

                const vibrationChartData = timeData.data.slice(-10).map((item: any) => ({
                    time: new Date(item.timestamp).toLocaleTimeString(),
                    value: item.vibration
                }));
                setVibrationData(vibrationChartData);

                const noiseChartData = timeData.data.slice(-10).map((item: any) => ({
                    time: new Date(item.timestamp).toLocaleTimeString(),
                    value: item.noise
                }));
                setNoiseData(noiseChartData);
            }
        } catch (err) {
            console.error('Error fetching time-based data:', err);
        }
    };
        ...
        <div className="min-h-screen bg-gray-900 text-white p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                    ...
                    {/* Time Period Summary */}
                    {aggregatedData && aggregatedData.aggregatedData && aggregatedData.aggregatedData.length > 0 && (
                        <div className="bg-gray-800 rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="bg-gray-900 rounded-lg p-4">
                                    ...
                                    <div className="text-lg font-bold text-orange-400">
                                        {aggregatedData.aggregatedData[0]?.avg_temperature?.toFixed(1)}°C
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        Max: {aggregatedData.aggregatedData[0]?.max_temperature?.toFixed(1)}°C
                                    </div>
                                </div>
                                <div className="bg-gray-900 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-2">Vibration</div>
                                    <div className="text-lg font-bold text-blue-400">
                                        {aggregatedData.aggregatedData[0]?.avg_vibration?.toFixed(1)}mm/s
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        Max: {aggregatedData.aggregatedData[0]?.max_vibration?.toFixed(1)}mm/s
                                    </div>
                                </div>
                                <div className="bg-gray-900 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-2">Noise</div>
                                    <div className="text-lg font-bold text-purple-400">
                                        {aggregatedData.aggregatedData[0]?.avg_noise?.toFixed(1)}dB
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        Max: {aggregatedData.aggregatedData[0]?.max_noise?.toFixed(1)}dB
                                    </div>
                                </div>
                                <div className="bg-gray-900 rounded-lg p-4">
                                    <div className="text-sm text-gray-400 mb-2">Readings</div>
                                    <div className="text-lg font-bold text-green-400">
                                        {aggregatedData.aggregatedData[0]?.readings_count}
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        Total samples
                                    </div>
